...
import { deleteMerchant, getAllMerchants } from '../store/slices/merchantSlice.ts';
import UpdateMerchant from '../components/UpdateMerchant.tsx';
...
interface EnhancedTableProps {
    numSelected: number;
    onRequestSort: (event: React.MouseEvent<unknown>, property: keyof Merchant) => void;
...
function EnhancedTableHead(props: EnhancedTableProps) {
    ...
    const createSortHandler = (property: keyof Merchant) => (event: React.MouseEvent<unknown>) => {
...
function EnhancedTableToolbar(props: EnhancedTableToolbarProps) {
    const { numSelected, selected, merchantsData, onSearch } = props;
    ...
    const handleBulkAction = (action: string) => {
        if (action === 'delete') {
            dispatch(deleteMerchant({ token: userState.token, ids: selected })).then(() => {
        ...
        >
            ...
            {numSelected > 0 ? (
                ...
                <Stack direction="row" justifyContent="center" alignItems="center" gap={2}>
                    ...
                    <AddMerchantModal open={isModalOpen} onClose={() => setModalOpen(false)} />
...
const Merchants = () => {
    ...
    const data: Merchant[] = state?.merchants;
    ...
    React.useEffect(() => {
        ...
        dispatch(getAllMerchants({ token: userState.token, params }))
    ...
    const handleConfirmDelete = () => {
        if (rowToDelete !== undefined) {
            dispatch(deleteMerchant({ token: userState.token, ids: [rowToDelete] })).then(() => {
                dispatch(getAllMerchants({ token: userState.token }))
            })
        }
        handleClose();
    ...
    return (
        <Paper elevation={0} sx={{ width: '100%', overflow: 'hidden' }}>
            <EnhancedTableToolbar numSelected={selected.length} selected={selected} merchantsData={data} onSearch={handleSearch} />
